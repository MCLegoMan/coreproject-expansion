...
public class CoreProjectExpansionBlueberryBushBlock extends PlantBlock implements Fertilizable {
    ...
    public CoreProjectExpansionBlueberryBushBlock(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState((BlockState)((BlockState)this.stateManager.getDefaultState()).with(AGE, 0));
    }

    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(CoreProjectExpansionItems.BLUEBERRY_BUSH);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (!bl && player.getStackInHand(hand).isOf(Items.BONE_MEAL)) {
            ...
            int j = 3 + world.random.nextInt(8);
            dropStack(world, pos, new ItemStack(CoreProjectExpansionItems.BLUEBERRY_BUSH, j + (bl ? 1 : 0)));
